...
\MathematicalDescription{
  ...
  \begin{equation}
    \underline{Y} \, \,  = \, \,  h(\underline{X}) \, \, = \, \, \sum_{\idx \in \Nset^{n_X}} \; \underline{a}_{\idx} \; \psi_{\idx}(\underline{X})
  \end{equation}
  where $n$ denotes the dimension of vector $\vect{X}$. For practical implementation, \emph{finite dimensional} polynomial chaoses have to be built. In other words, a suitable finite subset $\cA$ of $\Nset^{n_X}$ has to be selected. Using a specific enumeration strategy, a one-to-one mapping between natural integers $j$ and the multi-indices $\idx$ in $\Nset^{n_X}$ is created and the PC series rewrites:
  \begin{equation}
  ...
  % \begin{equation}
  %     \cA^{n,p} \, \, \equiv \, \, \left\{\idx \in \Nn \, : \, \|\idx \|_{1}  \leq p \right\}
  % \end{equation}
  ...
  %       \cA^{n,p}_{q} \, \, \equiv \, \,  \{\idx \in \Nn \, : \, \| \idx \|_{q} \leq p \}
  % \end{equation}
  % where:
  ...
  %
  % \begin{center}              \includegraphics[width = 9cm]{Figures/qnorms_2D_princ.pdf} \end{center}
  % \begin{center}       \includegraphics[width = 8cm]{Figures/qnorms_2D.pdf} \end{center}
  %
  % Note that setting $q$ equal to 1 corresponds to the usual truncation scheme, i.e. $\cA^{M,p}_{1} = \cA^{M,p}$. When using $q < 1$, the retained basis polynomials are located under an hyperbola, hence the name \emph{hyperbolic truncation scheme}.
