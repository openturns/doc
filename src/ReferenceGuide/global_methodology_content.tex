...
For the practical side i.e. the use of these functions in OpenTURNS Textual User Interface, the User is refered to the  \otrefext{OpenTURNS_UserManual_TUI}{TUI User Manual} and \otrefext{OpenTURNS_UseCasesGuide}{TUI Use Cases Guide}.
...
\paragraph{Same deterministic and probabilistic criteria as for random vectors}
\par
...
In the following, the input vector is considered as a stochastic process $\underline{X}(t)$. As previously, its probability distribution is required to perform an uncertainty study.
...
\subsection{Step C': Ranking uncertainty sources / Sensitivity analysis (only for random vectors)}
