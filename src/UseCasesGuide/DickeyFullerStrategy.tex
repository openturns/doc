...
\begin{tikzpicture}[%
  ...
  \tikzset{
    ...
    proc/.style={base, rectangle, text width=14em},
    test/.style={base, diamond, aspect=3, text width=14em},
  ...
  \node [proc, it] (p0)
        {
          \begin{tabular}{l}Model: $X_t = a + bt + \rho  X_{t-1} + \varepsilon_{t}$\\
            Estimation: $\hat{a}_n$, $\hat{b}_n$, $\hat{\rho}_n$, $\hat{\sigma}_{\rho_n}$\\
            Test statistics: $t_n=\frac{\hat{\rho}_n-1}{\hat{\sigma}_{\rho_n}}$
          \end{tabular}
        };
        % Use join to connect a node to the previous one
        \node [test, join] (t1) {$ \mathcal{H}_0: \rho = 1 \mbox{ vs } \mathcal{H}_1: \rho < 1$};
